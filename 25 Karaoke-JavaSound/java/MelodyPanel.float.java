...
public class MelodyPanel extends JPanel {
    ...
    private static int DBL_BUF_SCALE = 1;
    private static final int NOTE_HEIGHT = 10;
    private static final int SLEEP_MSECS = 50;
	...
	System.err.println("Max: " + maxNote + " Min " + minNote);
	...
	if (title != null) {
	    ...
	    System.out.println("Drawn title " + title);
	}
	...
	for (DurationNote note: notes) {
	    ...
	    long start = front + Math.round(startNote * DBL_BUF_SCALE);
	    long end = front + Math.round(endNote * DBL_BUF_SCALE);

	    drawNote(g, ht, start, end);
	    //g.drawString(title, (int)start, (int)height/2);	
	}
	} catch(Exception e) {
	    System.out.println("Drawing melody error " + e.toString());
	}
    }
	...
	System.err.printf("Drawing melody at start %d end %d height %d\n", start, end,  height - NOTE_HEIGHT/2);
	...
	if (image == null) {
	     ...
	     * A DBL_BUF_SCALE of 2.0 does this okay. But then tickLength * DBL_BUF_SCALE may end
	     ...
	     */
	    /*
	    if ((long) (tickLength * DBL_BUF_SCALE) > (long) Integer.MAX_VALUE) {
		DBL_BUF_SCALE = ((float)  Integer.MAX_VALUE) / ((float) tickLength);
		System.out.println("Adjusted DBL_BUF_SCALE to "+ DBL_BUF_SCALE);
	    }
	    */
	    System.out.println("DBL_BUF_SCALE is "+ DBL_BUF_SCALE);

	    // draw melody into a buffered image
	    System.err.printf("New buffered img width %d ht %d\n", tickLength, ht);
	    image = new BufferedImage(front + Math.round(tickLength * DBL_BUF_SCALE), ht, BufferedImage.TYPE_INT_RGB);
	    Graphics ig = image.getGraphics();
	    drawMelody(ig, front, Math.round(tickLength * DBL_BUF_SCALE), ht);
	    new TickPointer().start();
	...
	System.err.printf("Drawing img from %d ht %d width %d\n", 
			  front + Math.round(currentTick * DBL_BUF_SCALE - width/2), ht, width);
	
	boolean b = g.drawImage(image, 0, 0, width, ht, 
				front + Math.round(currentTick * DBL_BUF_SCALE - width/2), 0, 
				front + Math.round(currentTick * DBL_BUF_SCALE + width/2), ht,
		    null);
